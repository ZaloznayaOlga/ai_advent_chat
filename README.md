...
- **Сохранение истории** — история диалога сохраняется в локальной базе данных (Room)
- Информативные сообщения об ошибках на русском языке

### Суммаризация диалога
- **Автоматическая суммаризация** — при достижении порогов (по умолчанию: 10 сообщений или 4000 токенов)
- **Ручная суммаризация** — кнопка "Показать краткую историю диалога" появляется при входе в приложение, если есть несуммаризированные сообщения
- **Настройка порогов** — можно настроить количество сообщений и токенов для автоматической суммаризации
- **Оптимизация контекста** — суммаризированные сообщения заменяются кратким резюме для экономии токенов
- **Визуальное отображение** — резюме выделяется оранжевым цветом с информацией о количестве суммаризированных сообщений и токенов

### Настройки AI
- **Температура** — настройка креативности ответов (0.0 - 1.5)
- **Стиль общения** — общие ответы или с уточняющими вопросами
- **Суммаризация** — включение/отключение автоматической суммаризации, настройка порогов
...
- Резюме диалога с оранжевым фоном (при наличии суммаризации)
- Кнопкой "Показать краткую историю диалога" над полем ввода (при входе в приложение)
...
└── java/com/olgaz/aichat/
    ...
    │   │   ├── MessageEntity.kt        # Entity сообщения
    │   │   ├── SettingsDao.kt          # DAO для настроек
    │   │   └── SettingsEntity.kt       # Entity настроек
    │   ├── provider/
    ...
    │   └── repository/
    │       ├── ChatHistoryRepositoryImpl.kt # Репозиторий истории (Room)
    │       └── ChatRepositoryImpl.kt   # Repository + JSON/XML parsing
    │
    ├── domain/                         # Domain Layer
    │   ├── model/
    │   │   ├── Message.kt              # Domain model + MessageJsonData + MessageMetadata + SummarizationInfo
    │   │   └── ChatSettings.kt         # Настройки: провайдер, модель, температура, суммаризация
    │   ├── provider/
    │   │   └── SystemPromptProvider.kt # Интерфейс провайдера промпта
    │   ├── repository/
    │   │   ├── ChatHistoryRepository.kt # Интерфейс репозитория истории
